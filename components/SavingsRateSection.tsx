const SavingsRateSection = () => {
  return (
    <div className="bg-gradient-to-br from-indigo-600 to-indigo-800 rounded-3xl p-6 shadow-xl text-white relative overflow-hidden flex flex-col justify-between">
      ...
      <div className="relative z-10 py-4">
        <div className="flex items-baseline gap-2">
          ...
          <div className="bg-indigo-400/20 text-indigo-300 px-2 py-1 rounded-lg text-xs font-bold">
            +4.2%
          </div>
        </div>
      ...
      <div className="relative z-10 pt-4 border-t border-white/10 flex justify-between items-center">
        ...
        <div className="flex gap-1">
          {[1, 2, 3, 4, 5].map((i) => (
            <div key={i} className={`w-1.5 h-1.5 rounded-full ${i < 5 ? 'bg-indigo-400' : 'bg-white/20'}`} />
          ))}
        </div>
      </div>
    </div>
